...
<form class="wc-progress-form-content woocommerce-importer" method="post" action="<?php echo esc_url( $this->get_next_step_link() ) ?>">
    <header>
        <h2><?php esc_html_e( 'Map CSV fields to products', 'dpi_plugin' ); ?></h2>
        <p><?php esc_html_e( 'Select fields from your CSV file to map against products fields, or to ignore during import.', 'dpi_plugin' ); ?></p>
    </header>
    <section class="wc-importer-mapping-table-wrapper">
        <table class="widefat wc-importer-mapping-table">
            <thead>
                <tr>
                    <th><?php _e( 'Column name', 'dpi_plugin' ); ?></th>
                    <th><?php _e( 'Map to field', 'dpi_plugin' ); ?></th>
                </tr>
            </thead>
            <tbody>
                    ...
                    <tr>
                        <td class="wc-importer-mapping-table-name">
                            ...
                            <?php if ( !empty( $sample[$index] ) ) : ?>
                                <span class="description"><?php _e( 'Sample:', 'dpi_plugin' ); ?> <code><?php echo esc_html( $sample[$index] ); ?></code></span>
                            <?php endif; ?>
                        </td>
                        <td class="wc-importer-mapping-table-field">
                            ...
                            <select name="map_to[<?php echo esc_attr( $index ); ?>]">
                                <option value=""><?php esc_html_e( 'Do not import', 'dpi_plugin' ); ?></option>
                                <option value="">--------------</option>
    ...
    <div class="wc-actions">
        <input type="submit" class="button button-primary button-next" value="<?php esc_attr_e( 'Run the importer', 'dpi_plugin' ); ?>" name="save_step" />
